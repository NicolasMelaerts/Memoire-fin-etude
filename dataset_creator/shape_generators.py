...
class ShapeGenerator:
    ...
    def _create_triangle_points(self, center_x, center_y, img_w, img_h):
        """Create equilateral triangle points. Returns None if triangle goes out of bounds."""
        ...
        for i in range(3):
            ...
            if pt_x < 0 or pt_x >= img_w or pt_y < 0 or pt_y >= img_h:
                return None, None  # Triangle would be clipped - reject it
    ...
    def draw_arrow(self, draw, start, end, color=0):
        """Draw a crisp arrow with better rendering."""
        # Convert to integers for pixel-perfect rendering
        start = (int(round(start[0])), int(round(start[1])))
        end = (int(round(end[0])), int(round(end[1])))

        # Draw main line with specified width
        ...
        # Calculate arrow head points with integer coordinates
        ...
        # Add extra lines to make arrow head thicker and more visible
    ...
    def add_random_arrow(self, draw, w, h, target_center=None, target_radius=None, force_arrow=False):
        """Add an arrow to the image if force_arrow is True. Retry position until it works."""
        if not force_arrow:
            return True, False  # Success (valid run), but no arrow
        ...
        for _ in range(50):
            ...
            if not (0 <= start_x < w and 0 <= start_y < h):
                ...
                if not (0 <= start_x < w and 0 <= start_y < h):
                    angle = angle + math.pi  # Try opposite direction
                    if target_center and target_radius:
                        end_x = target_center[0] + target_radius * math.cos(angle)
                        end_y = target_center[1] + target_radius * math.sin(angle)
                        start_x = target_center[0] + (target_radius + arrow_len) * math.cos(angle)
